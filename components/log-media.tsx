...
export function LogMedia({ media, featured = false }: { media: Media; featured?: boolean }) {
  ...
  if (media.kind === 'video') return (
    <figure className="video-media">
      <video controls playsInline preload="metadata" poster={assetPath(media.poster)} aria-label={media.alt} onError={() => setFailed(true)}>
        <source src={assetPath(media.src)} />
        {media.captions && <track default kind="captions" src={assetPath(media.captions)} srcLang="ko" label="한국어" />}
  ...
  return (
    <Dialog>
      <DialogTrigger className="media-trigger" aria-label={media.alt + ' — 크게 보기'}>
        <img src={assetPath(media.src)} alt={media.alt} width={media.width} height={media.height} loading={featured ? 'eager' : 'lazy'} fetchPriority={featured ? 'high' : 'auto'} onError={() => setFailed(true)} />
      ...
      <DialogContent showCloseButton={false} className="lightbox">
        ...
        <img src={assetPath(media.src)} alt={media.alt} width={media.width} height={media.height} />
