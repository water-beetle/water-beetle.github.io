...
우주 생활 게임 개발 기록. 날짜별 글 목록과 개별 읽기 페이지, 최적화 메모를 제공합니다. 우주 배경과 어두운 색감에 한국어 본문과 실제 작업 이미지를 중심으로 구성합니다.
...
4. 글 주소는 `/devlog/글-id/`입니다. 기존 `/#글-id`도 목록의 해당 항목으로 연결됩니다. 본문 섹션의 `media` 배열에 이미지를 넣으면 캡션과 확대 보기가 표시됩니다. 첫 문단은 `title: ''`로 소제목 없이 시작할 수 있습니다.
...
- 개별 글에서 `article_view` 이벤트를 보냅니다. 글 제목이 화면에 50% 이상 나타난 채 2초가 지나면 한 번 기록하며, 빠르게 지나치거나 숨겨진 탭은 제외합니다. 목록에는 글 조회 표식과 댓글 위젯을 넣지 않습니다. 완독 수치는 아닙니다. `article_id`, `article_title`, `article_type` 매개변수를 사용하며 페이지 조회수와 별도로 해석합니다.
...
- 댓글은 `Announcements` 카테고리에 저장되며 첫 댓글이 달릴 때 해당 글의 Discussion이 생성됩니다. 개발일지의 실제 주소는 바뀌었지만 댓글의 고유 키는 기존 `/#글-id`를 유지합니다. 최적화 글은 `/optimization/글-slug/`를 사용합니다. 기존 id와 slug는 바꾸지 마세요.
...
2026년 9월 25일 편집 개편: 개발일지 14편의 제목·도입·본문을 작업 기록 문체로 다시 쓰고, 최적화 메모의 제목·목차·요약을 정리했습니다. 측정값과 코드·출처는 유지합니다. 홈의 반복 카드와 두 열 전체 본문을 없애고 `/devlog/[slug]/`에 개별 글을 제공합니다. 영어 장식 문구를 줄였으며, 과거 링크의 앵커와 댓글 식별자는 보존합니다. 후속 피드백으로 기존 별 배경과 어두운 색감을 복원했습니다.
